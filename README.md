...
Vegetation, SPEI, environmental predictor, and trend rasters should be mapped to the corresponding manuscript variables according to the file-to-variable mappings documented in the relevant script header comments. For the fixed-month analysis, the generation and post-processing steps are implemented in `generate_joint_Rmax_Topt_fixed_month_Topt.py` and `analyze_fixed_month_Topt_selected_month.py`, respectively.
...
6. **Generate fixed-month Topt and selected-month products**
   - For each moving window, track the calendar month associated with the joint maximum vegetation–SPEI correlation.
   ...
   - Save the resulting fixed-month Topt and selected-month products for subsequent analysis.
   - This step is implemented in `generate_joint_Rmax_Topt_fixed_month_Topt.py`.

7. **Analyze fixed-month Topt and selected-month dynamics**
   - Retain only pixels with valid fixed-month Topt estimates across all 24 17-year moving windows.
   - Calculate fixed-month Topt trends using Theil–Sen slopes and the Hamed–Rao modified Mann–Kendall test.
   - Summarize the temporal proportions and switching behavior of the selected calendar month.
   - Generate the fixed-month Topt trend summary reported in Table S4.
   - This step is implemented in `analyze_fixed_month_Topt_selected_month.py`.

8. **XGBoost–SHAP analysis**
...
9. **Figure and table preparation**
...
- **Table S4**: fixed-month Topt trend analysis across April–October. The fixed-month Topt products were generated by `generate_joint_Rmax_Topt_fixed_month_Topt.py` and subsequently analyzed using `analyze_fixed_month_Topt_selected_month.py`.
- **Selected-month diagnostics**: monthly proportions and switching statistics were generated using `analyze_fixed_month_Topt_selected_month.py`.
